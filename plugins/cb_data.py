...
async def _prepare_thumb(client: Client, user_id: int):
    ...
    try:
        th_msg = await client.download_media(
            await client.get_profile_photos(user_id, limit=1).next(),
            file_name=path
        )
...
# Optimized download with visible progress bar
async def _download_with_progress(client, message, file_name, status_msg):
    c_time = time.time()
    return await client.download_media(
        message,
        file_name=file_name,
        block=True,  # ensure chunk processing
...
async def do_upload(client: Client, query):
    ...
    new_name = _parse_new_name(msg.text or msg.caption or "") \
        or media.file_name \
        or f"file.{('mp4' if kind == 'video' else 'bin')}"
    ...
    await _download_with_progress(client, src, dl_path, status)
